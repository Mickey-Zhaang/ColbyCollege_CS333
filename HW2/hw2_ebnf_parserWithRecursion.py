# hw2_ebnf_parserWithRecursion.py
# Stephanie R Taylor Fall 2025
# Parse a very simple expression grammar expressed in EBNF
# This grammar is similar to that in hw2_ebnf_parser.py, but this grammar
# includes recursion because it also allows parentheses to be used to increase
# the precedence of a sub-expression
# The grammar involves a single expression with integers, addition, and subtraction
# The test code uses the string "3-23-(4+7)" and creates a tree
#
#                           ExpressionNode
#                       left      op=-     right
#                         /                        \
...
#        /                      |                    |                      \
#     /                         |                    |                       \
#  IntegerNode       IntegerNode   IntegerNode        IntegerNode
#   value=3              value=23          value=4               value=7
#
...
class Parser:
    ...
    def parsePrimary( self ):
        # Is either an Integer or an expression between parentheses
        # So figure out which by looking at the current token
        if self.token.value == "(":
            self.getToken() # move past the (. it has done its job of getting us into this if-branch
            exp = self.parseExpression()
            if self.token.value != ")":
                print( "expecting a close parenthesis, but found:", self.token )
            self.getToken() # move past the )
            return exp
        # If we didn't already return with parenthetical expression, then it must an integer
    ...
    def parseExpression( self ):
        ...
        while self.token is not None and self.token.value in "+-":
            ...
            c = ExpressionNode( c, op, self.parsePrimary() )
        return c       
